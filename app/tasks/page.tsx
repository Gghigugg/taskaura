...
export default async function TasksPage() {
  ...
  const { data: tasks, error } = await supabase
    .from('tasks')
    .select('id,name,description,category,reward,estimated_minutes,proof_required,deadline,status')
    .eq('status', 'active')
    .order('created_at', { ascending: false })

  return (
    <main className="tasks-page">
      <header><Link href="/dashboard">← Dashboard</Link><p>TASKAURA</p><h1>Available Tasks</h1><span>Complete verified tasks and earn rewards.</span></header>
      <section className="task-list">
        {error ? <div className="empty"><div>⚠️</div><h2>Tasks are temporarily unavailable</h2><p>Please try again shortly.</p></div> : tasks?.length ? tasks.map((task) => (
          <article className="task-card" key={task.id}>
            ...
            <div className="meta"><span>⏱ {task.estimated_minutes ? `${task.estimated_minutes} min` : 'Flexible'}</span><span>🧾 {task.proof_required ? 'Proof required' : 'No proof'}</span>{task.deadline && <span>📅 {new Date(task.deadline).toLocaleDateString()}</span>}</div>
