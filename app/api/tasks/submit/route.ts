...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (task.proof_required && (!(file instanceof File) || file.size === 0)) return NextResponse.json({ error: 'Proof file is required for this task.' }, { status: 400 })
    ...
    if (file instanceof File && file.size > 0) {
      ...
      path = `${authUser.id}/${taskId}/${crypto.randomUUID()}-${safeName}`
    ...
    if (insertError) {
      if (path) await supabase.storage.from('task-proofs').remove([path])
      if (insertError.code === '23505') return NextResponse.json({ error: 'You already have a submission for this task.' }, { status: 409 })
